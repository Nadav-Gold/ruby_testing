...
describe BinaryGame do
  ...
  describe '#verify_input' do
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        min = game_verify.instance_variable_get(:@minimum)
        max = game_verify.instance_variable_get(:@maximum)
        invalid_input = max + 1 # Above max is invalid
  ...
  describe '#display_turn_order' do
    ...
    let(:display_search_turn) { instance_double(BinarySearch) }
    subject(:game_turn) { described_class.new(0, 100) }

    before do
      # You'll need to create a few method stubs.
      allow(display_search_turn).to receive(:make_guess)
      allow(display_search_turn).to receive(:update_range)
      allow(game_turn).to receive(:display_guess).with(display_search_turn)
    end

    # Command Method -> Test the change in the observable state
    it 'increases guess_count by one' do
      game_turn.display_turn_order(display_search_turn)
      guess_count = game_turn.instance_variable_get(:@guess_count)
      expect(guess_count).to eq(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(display_search_turn).to receive(:make_guess).once
      game_turn.display_turn_order(display_search_turn)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends update_range' do
      expect(display_search_turn).to receive(:update_range).once
      game_turn.display_turn_order(display_search_turn)
    end
