...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 5 and max is 9' do
      subject(:guess_seven) { described_class.new(5, 9, number_guessing) }

      it 'returns 7' do
        expect(guess_seven.make_guess).to eq(7)
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:guess_eight) { described_class.new(8, 9, number_guessing) }

      it 'returns 8' do
        expect(guess_eight.make_guess).to eq(8)
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:guess_one) { described_class.new(0, 3, number_guessing) }

      it 'returns 1' do
        expect(guess_one.make_guess).to eq(1)
      end
    end

    context 'when min and max both equal 3' do
      subject(:guess_three) { described_class.new(3, 3, number_guessing) }

      it 'returns 3' do
        expect(guess_three.make_guess).to eq(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    let(:random_number) { double('random_number', value: 10) }
    context 'when guess and random_number are equal' do
      ...
      subject(:complete_game) { described_class.new(0, 20, random_number, random_number.value) }

      it 'is game over' do
        expect(complete_game).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      subject(:incomplete_game) { described_class.new(0, 20, random_number, random_number.value + 1) }

      it 'is not game over' do
        expect(incomplete_game).not_to be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:max_guess_game) { described_class.new(5, 8, number_range, 7) }

      before do
        max_guess_game.update_range
      end

      it 'updates min to the same value as max' do
        minimum = max_guess_game.min
        maximum = max_guess_game.max
