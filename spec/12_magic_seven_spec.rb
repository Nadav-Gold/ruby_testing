...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context 'when the previous step is 10 and random_number is 4' do
      subject(:game_four) { described_class.new(4) }

      it 'returns 6' do
        previous_step = 10
        result = game_four.subtract_random_number(previous_step)
  ...
  describe '#play' do
    context 'when the random number is 1000000' do
      # remove the 'x' before running this test
      subject(:game_milion) { MagicSeven.new(1_000_000) }

      it 'will return 7' do
        result = game_milion.play
    ...
    context 'when the random number is 7' do
      # remove the 'x' before running this test
      subject(:game_seven) { MagicSeven.new(7) }

      it 'will return 7' do
        result = game_seven.play
        expect(result).to eq(7)
      end
    end
    ...
    context 'when the random number is 0' do
      # remove the 'x' before running this test
      subject(:game_zero) { MagicSeven.new(0) }
